...
void CMotion::SetMotion(const int nCntMotionSet)
{
	...
	for (int nCntParts = 0; nCntParts < m_nMaxParts; nCntParts++)
	{
		...
		// 角度の正規化
		rot.x = CUtility::GetNorRot(rot.x);
		rot.y = CUtility::GetNorRot(rot.y);
		rot.z = CUtility::GetNorRot(rot.z);
...
void CMotion::PlayMotion()
{
	...
	for (int nCntParts = 0; nCntParts < m_nMaxParts; nCntParts++)
	{
		...
		{// フレームカウントが0の時
			...
			// 角度の正規化
			rotDest.x = CUtility::GetNorRot(rotDest.x);
			rotDest.y = CUtility::GetNorRot(rotDest.y);
			rotDest.z = CUtility::GetNorRot(rotDest.z);
